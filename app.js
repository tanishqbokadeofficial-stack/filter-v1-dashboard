...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const currentGroupExported = document.getElementById("current-group-exported");
    ...
    const detailInviteLink = document.getElementById("detail-invite-link");
    const detailGroupType = document.getElementById("detail-group-type");
    const detailHistoryVisible = document.getElementById("detail-history-visible");
    const detailTopicsEnabled = document.getElementById("detail-topics-enabled");
    const detailMessagingAllowed = document.getElementById("detail-messaging-allowed");
    const detailReactionsEnabled = document.getElementById("detail-reactions-enabled");
    const detailMediaRestricted = document.getElementById("detail-media-restricted");
    ...
    const detailScanned = document.getElementById("detail-scanned");
            ...
            card.innerHTML = `
                ...
                <div class="group-card-subtitle">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ${metricHtml}
                        <a href="${g.link}" target="_blank" class="group-card-link-icon" onclick="event.stopPropagation();" title="Open Telegram Link">
    ...
    async function selectGroup(group) {
        ...
        currentGroupExported.style.display = "none";
        ...
        if (detailInviteLink) {
            const linkVal = meta.invite_link || group.link || "None";
            detailInviteLink.textContent = linkVal;
            detailInviteLink.href = linkVal.startsWith('http') ? linkVal : `https://${linkVal}`;
        }
        if (detailGroupType) {
            detailGroupType.textContent = meta.group_type || group.link_type || "Unknown";
        }
        if (detailHistoryVisible) {
            detailHistoryVisible.textContent = meta.history_visible === true ? "Visible" : (meta.history_visible === false ? "Hidden" : "Unknown");
        }
        if (detailTopicsEnabled) {
            detailTopicsEnabled.textContent = meta.topics_enabled === true ? "Yes" : (meta.topics_enabled === false ? "No" : "Unknown");
        }
        if (detailMessagingAllowed) {
            detailMessagingAllowed.textContent = meta.messaging_allowed === true ? "Allowed" : (meta.messaging_allowed === false ? "Restricted" : "Unknown");
        }
        if (detailReactionsEnabled) {
            detailReactionsEnabled.textContent = meta.reactions_enabled === true ? "Enabled" : (meta.reactions_enabled === false ? "Disabled" : "Unknown");
        }
        if (detailMediaRestricted) {
            detailMediaRestricted.textContent = meta.media_restrictions_exist === true ? "Yes" : (meta.media_restrictions_exist === false ? "No" : "Unknown");
        }
        ...
        if (group.scanned_at) {
            const scannedDate = new Date(group.scanned_at);
            detailScanned.textContent = scannedDate.toLocaleString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        } else {
            detailScanned.textContent = "Unknown";
        }
        
        detailJobId.textContent = group.job_id || "None";
    ...
    async function fetchChatHistory(link) {
        ...
        try {
            ...
            if (data.exported_at) {
                const expDate = new Date(data.exported_at);
                currentGroupExported.innerHTML = `<i class="fa-solid fa-clock-rotate-left"></i> Exported: ${expDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} at ${expDate.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}`;
                currentGroupExported.style.display = "inline-flex";
            } else {
                currentGroupExported.style.display = "none";
            }
    ...
    function renderChatMessages(messagesList) {
        ...
        chronologicalMessages.forEach(msg => {
            ...
            bubble.innerHTML = `
                <span class="message-sender ${colorClass}">
                    ${escapeHTML(msg.sender)}
                    ${msg.sender_id ? `<small style="font-size: 0.65rem; color: var(--text-muted); font-weight: normal; margin-left: 6px;">(ID: ${msg.sender_id})</small>` : ''}
                </span>
                <div class="message-text">${formattedBody}</div>
                <div class="message-footer">
                    <span class="message-id" style="font-size: 0.65rem; color: var(--text-muted); margin-right: auto; padding-right: 10px;">#${msg.id}</span>
    ...
    function formatMessageText(text) {
        ...
        escaped = escaped.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, linkText, url) => {
            ...
            let href = url;
            placeholders.push(`<a href="${href}" target="_blank" rel="noopener noreferrer">${linkText}</a>`);
            return `__HTML_PLACEHOLDER_${id}__`;
        });
        
        // 2. Extract markdown inline code `code`
        escaped = escaped.replace(/`([^`]+)`/g, (match, codeText) => {
            const id = placeholders.length;
            placeholders.push(`<code class="chat-code">${codeText}</code>`);
            return `__HTML_PLACEHOLDER_${id}__`;
        });

        // 3. Extract bold **text** or __text__
        escaped = escaped.replace(/\*\*([^*]+)\*\*/g, (match, boldText) => {
            const id = placeholders.length;
            placeholders.push(`<strong>${boldText}</strong>`);
            return `__HTML_PLACEHOLDER_${id}__`;
        });

        // 4. Unified non-overlapping regex for HTTP/S urls, raw t.me links, and @mentions
        const pattern = /(https?:\/\/[^\s<]+)|(\bt\.me\/[A-Za-z0-9_+\-]+)|((?<!\w)@[A-Za-z0-9_]{4,32}\b)/ig;
        escaped = escaped.replace(pattern, (match, p1, p2, p3) => {
            const id = placeholders.length;
            if (p1) {
                placeholders.push(`<a href="${p1}" target="_blank" rel="noopener noreferrer">${p1}</a>`);
            } else if (p2) {
                placeholders.push(`<a href="https://${p2}" target="_blank" rel="noopener noreferrer">${p2}</a>`);
            } else if (p3) {
                const username = p3.substring(1);
                placeholders.push(`<a href="https://t.me/${username}" target="_blank" rel="noopener noreferrer">${p3}</a>`);
            }
            return `__HTML_PLACEHOLDER_${id}__`;
        });
        ...
        for (let i = 0; i < placeholders.length; i++) {
            escaped = escaped.replace(`__HTML_PLACEHOLDER_${i}__`, placeholders[i]);
        }
